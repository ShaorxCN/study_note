...
- 输出部分:紧接指令后面，记录指令部分的输出信息，格式:`"输出操作约束"(输出表达式),"输出操作约束"(输出表达式),...`约束和表达式成对出现。(补充下 0-9约束符表示和第几个操作数使用相同的寄存器)
...
- 损坏部分:描述指令执行过程中被修改过的寄存器，内存空间或者标志位寄存器。并且修改部分没有再输出部分或者输入部分出现过。格式:`"损坏描述","损坏描述",...`
    - 寄存器修改通知:example:<br>`__asm__ __volatile__ ("movl %0, %%ecx"::"a"(__tmp):"cx")`;<br>这里使用输入部分`__tmp`修改了ecx的值。但是没有再输入输出部分说明(输出部分省略，输入部分没有说明。那么就在损坏部分说明)。这里说明下这段代码。首先 代码指令本身没啥问题，`%0`代表引用c语言变量的操作数占位符，也就是第一个`()`里指定的。然后`""`是对操作数的约束。这里是`a`。表示`rax`也就是将`__tmp`放入`rax`在使用作为输入复制到ecx。当然这里movl说明是mov双字也即是32位 自然是ecx。**损坏部分声明的寄存器不能作为输入输出**（部分限制符参见下图）
...
mov指令置位(pe(bit[0])，pg(bit[31] 分页可选))。处理器从0特权级开始执行。推荐遵循顺序依次如下:
...
3. MOV CR0汇编指令位置CR0的PE(bit[0])标志位
...
IA32-EFER寄存器（位于MSR寄存器组内:MSR是在x86架构处理器中，一系列用于控制CPU运行、功能开关、调试、跟踪程序执行、监测CPU性能等方面的寄存器，可以分别通过RDMSR和WRMSR 两条指令进行读和写的操作，前提要在ECX中写入MSR的地址）的LME标志位用于控制IA-32e的开启和关闭。处理器重启该位也会重置清零。该模式下页管理机制是四级页表结构 。该模式激活前 cr0.pg=1 他的cr3只又低32位可写入数据 也就是4gb的寻址范围。激活该模式后 可以重新定位页表到物理内存的任何地点。下面是标准初始化步骤：（关分页->拓展分页->开启IA-32e模式->开启分页）

1. 保护模式下 mov指令重置cr0的PG标志位 关闭分页（此后的指令必须位于同一性地址映射页面内）
2. 置位CR4的PAE(5bit位)标志位，开启物理地址拓展功能
3. 将页目录（PML4）的物理基地址加载到cr3中
4. 置位IA32_EFER寄存器的LME(bit[8])的标志位  开启IA-32e模式
5. 置位cr0控制寄存器的PG标志位开启分页机制。处理器自动置位IA32_EFER的LMA(bit[10])位（1 表示当前运行在IA-34e模式）当mov cr0开启分页的时候  其后续指令必须位于同一性地址映射的页面。  知道处理器进入IA-32e模式后

当试图修改类似LME PG PAE等标志位时  处理器会进行64位模式的一致性检测,确保模式正常 如果检测失败 则引发通用保护性异常（#GP）。一下会导致一致性检测失败:

- 开启分页后 再试图是能或者禁止IA-32e （也就是不关分页尝试模式切换）
- 开启IA-32e 后 尝试未开启PAE就开启分页功能
- 激活IA-32e后，试图禁止PAE
- CS寄存器的L被置位 再试图激活IA-32e（新增的L属性  指示当前处于64位模式还是兼容模式 =1时代表时64位）
- TR寄存器加载的时16位TSS段结构
