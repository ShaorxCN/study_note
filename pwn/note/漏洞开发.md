...
基于这发现覆盖的值至少为4因为地址就至少4字节的计数。但是可以通过将地址放在末尾实现小于4的覆盖 因为%n只计算他出现的时候的。`AA%15$nA\xd8\xcf\xff\xff` 但是这因为地址后置 所以覆盖的位置也变了 这边AA 2B 然后%15$nA 6B 刚好8B 两个参数的宽度 所以这边是13+2 第15个arg 对于大数字 上面0x8048000的例子证明是可以直接写的 但是因为太过占用内存容易出现问题 可能会覆盖一些关键地址 所以可以通过逐字节覆盖的方法
